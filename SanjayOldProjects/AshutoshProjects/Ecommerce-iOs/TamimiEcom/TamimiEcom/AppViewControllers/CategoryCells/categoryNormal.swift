...
import UIKit
import Kingfisher
protocol LikeCellRemoveNormalDelegate : class {
    func isLikeRemovedNormal(index:Int)
    func openLoginViewNormal()
}
class categoryNormal: UICollectionViewCell {
    weak var likeRemoveDeleagte :LikeCellRemoveNormalDelegate?
                    ...
                    if let code = valueData["code"] as? Int {
                        if code == 201 {
                            ...
                            if !self.categoryModel.isLiked  && self.likeRemoveDeleagte != nil {
                                self.likeRemoveDeleagte?.isLikeRemovedNormal(index: self.btnLike.tag)
                            }
                        }
                    }
                }
