...
extension UIViewController {
    ...
    func createToolbarPicker(_ type: ToolbarPickerType) -> UIToolbar {
        ...
        switch type {
        case .gender:
            done = UIBarButtonItem(title: LanguageManager.get("toolbar.done"), style: .plain, target: self, action: #selector(genderPickerTouchUpInside(_:)))
        case .date:
            done = UIBarButtonItem(title: LanguageManager.get("toolbar.done"), style: .plain, target: self, action: #selector(datePickerTouchUpInside(_:)))
        }
        let cancel = UIBarButtonItem(title: LanguageManager.get("toolbar.cancel"), style: .plain, target: self, action: #selector(cancelPickerTouchUpInside(_:)))
