...
class MoreOptionsViewController: UIViewController {
    ...
    func updateCatType() {
        if categoryType == 0 {
            self.categoryName.text = "EXCLUSIVE DEALS"
            self.categoryNameValue = "exclusive"
        }else  if categoryType == 1 {
            self.categoryNameValue = "weekly"
            self.categoryName.text = "WEEKLY OFFERS"
        }else if categoryType == 2 {
            self.categoryNameValue = "bundle"
            self.categoryName.text = "BUNDLE OFFERS"
        }
        else if categoryType == 4 {
            self.categoryNameValue = "hot_deal"
            self.categoryName.text = "HOT DEALS"
        }
        else if categoryType == 5 {
            self.categoryNameValue = "new_arrival"
            self.categoryName.text = "NEW ARRIVAL"
        }
    }
...
extension MoreOptionsViewController {
    func getDetailsData() {
        let params : [String : String] = ["tag":self.categoryNameValue]
        
        NetworkManager.shared.postJSONResponse(path: Constants.fetchProducts, parameters: params) { (value, status) in
            switch status {
            case .success:
...
extension MoreOptionsViewController : LikeCellRemoveDelegate,LikeCellRemoveNormalDelegate {
    func openLoginView() {
        //Do Nothing
    }
