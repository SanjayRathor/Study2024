...
class MakeCampaignViewController: UIViewController {
    ...
    @IBAction func chooseTeamADidClicked(_ sender: Any) {
      
        let storyboard = UIStoryboard(storyboard: .main)
              let customAlert = storyboard.instantiateViewController(withIdentifier: "TeamsViewController") as! TeamsViewController
              customAlert.teams  = campaignInfo?.result.teams.teamA
              customAlert.teamType = 1
...
extension MakeCampaignViewController : TeamsViewControllerDelegate {
    func teamButtonTapped(teamId: Int, teamName: String, imageURL: String, teamType: Int) {
        ...
        if teamType == 0 {
            ...
            self.firstTeamId = "\(teamId)"
            self.teamALabel.text =  teamName
            let url = URL(string: (imageURL))
