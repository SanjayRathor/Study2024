//
//  CampaignsVotingsViewController.swift
//  FandomLive
//
...
import UIKit
class CampaignsVotingsViewController: SRBaseViewController {
    
    fileprivate let kTrendingCollectionIdentifier:String = "TrandingCollectionViewCell"
    
    var jsonModelItem:FLCampaignVotingModel? = nil
    var favoriteItems:[CampaignResult] = []
    ...
    func prepareItemSize() {
        ...
        if let flowLayout = self.collectionView.collectionViewLayout as? UICollectionViewFlowLayout {
            ...
            flowLayout.itemSize =  CGSize(width: CGFloat(itemWidth), height: 310);
        }
    }
}


extension CampaignsVotingsViewController : UICollectionViewDelegate,UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if section == 0 && favoriteItems.count > 0 {
            return favoriteItems[0].votingcampaign?.count ?? 0;
        }
        return 0;
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let votingArray = self.favoriteItems[indexPath.section].votingcampaign {
            let votingcampaign = votingArray[indexPath.row]
            cell.configureVotingCell(cellModel: votingcampaign)
...
extension CampaignsVotingsViewController {
    
    func getCampaigns(userId:String) {
        self.presentCustomActivity()
        
        let otpURLString = String(format: API.getVotingCampaignListURL, userId)
        SRDataManager.sharedInstance().performNetworkGETServiceRequestWithData(requestURL: otpURLString) { (result) -> Void in
            ...
            case .success(let jsonData):
                ...
                do {
                    self.jsonModelItem = try JSONDecoder().decode(FLCampaignVotingModel.self, from: jsonData as! Data)
                } catch let error {
                    print(error)
                }
