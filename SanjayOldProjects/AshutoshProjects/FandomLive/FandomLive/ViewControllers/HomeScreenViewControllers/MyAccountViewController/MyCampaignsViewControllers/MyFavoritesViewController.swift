...
class MyFavoritesViewController: SRBaseViewController {
    
    fileprivate let kTrendingCollectionIdentifier:String = "TrandingCollectionViewCell"
    
    var jsonModelItem:FLMyFavoritesModel? = nil
    ...
    func prepareItemSize() {
        ...
        if let flowLayout = self.collectionView.collectionViewLayout as? UICollectionViewFlowLayout {
            ...
            flowLayout.itemSize =  CGSize(width: CGFloat(itemWidth), height: 300);
        }
    }
}
...
extension MyFavoritesViewController : UICollectionViewDelegate,UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if section == 0 && favoriteItems.count > 0 {
            return favoriteItems[0].voting?.count ?? 0;
        }
        return 0;
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let votingArray = self.favoriteItems[indexPath.section].voting {
            let votingcampaign = votingArray[indexPath.row]
            cell.configureMyFavoritesCell(cellModel: votingcampaign)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let votingArray = self.favoriteItems[indexPath.section].voting {
            let votingcampaign = votingArray[indexPath.row]
            self.goToVotingsVC(campaignId: votingcampaign.campID)
        }
...
extension MyFavoritesViewController {
    ...
    func getCampaigns(userId:String) {
        ...
        let otpURLString = String(format: API.getmyFavoritesURL, userId)
        SRDataManager.sharedInstance().performNetworkGETServiceRequestWithData(requestURL: otpURLString) { (result) -> Void in
            ...
            case .success(let jsonData):
                ...
                do {
                    self.jsonModelItem = try JSONDecoder().decode(FLMyFavoritesModel.self, from: jsonData as! Data)
                } catch let error {
                    print(error)
                }
