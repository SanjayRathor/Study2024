...
extension TeamsViewController:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let followed = teams?[indexPath.row] as? Team {
            self.delegate?.teamButtonTapped(teamId: followed.id, teamName: followed.name, imageURL: followed.teamLogo)
            
        }
        
        if let followed = teams?[indexPath.row] as? Followed {
            self.delegate?.teamButtonTapped(teamId: followed.id, teamName: followed.name, imageURL: followed.imagePath ?? "")
