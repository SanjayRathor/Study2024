...
//
//   let fLDashBoard = try? newJSONDecoder().decode(FLDashBoard.self, from: jsonData)
...
@objcMembers class Results: NSObject, Codable {
    ...
    init(banner: [Banner], news: [News], securitycampaign: [JSONAny], socialcampaign: [JSONAny], votingcampaign: [Votingcampaign]) {
        self.banner = banner
        self.news = news
...
@objcMembers class News: NSObject, Codable {
    let date, id, image, title: String

    init(date: String, id: String, image: String, title: String) {
        self.date = date
        self.id = id
        self.image = image
        self.title = title
